...
int main(){
    ...
    */
    ...
    //Test Schedule packet 2
    Ptr<Packet> scheduledPkt2 = drr->testDequeue();

    Ipv4Header ipHeaderExtract2;
    TcpHeader tcpHeaderExtract2;

    

    if (scheduledPkt2) {
        scheduledPkt2->RemoveHeader(ipHeaderExtract2);
        scheduledPkt2->RemoveHeader(tcpHeaderExtract2);

        cout << "Scheduled a packet successfully!" << endl;
        cout << "Destination IP: " << ipHeaderExtract2.GetDestination() << endl;
        cout << "Protocol: " << (uint16_t)ipHeaderExtract2.GetProtocol() << endl;
        cout << "Destination port: " << tcpHeaderExtract2.GetDestinationPort() << endl;
    } else {
        cout << "Schedule returned nullptr — all queues empty." << endl;
    }   
